...
terraform {
    backend "s3" {
         ...
         key    =  "state/ITCA/env2.tfstate"
         region = "us-east-1"
 }
}

#Accessing State files for env3-state for configuration details
data "terraform_remote_state" "env2-state" {
   backend = "s3"
...
resource "aws_instance" "ec2-instance-itca" {
  ...
  tags        =  "${merge(module.global_static_vars.tags-env, map("Environment" , "ITCA"))}"
  vpc_security_group_ids  = "${aws_security_group.sg-itca.id}"
}
  
#Creating security group
resource "aws_security_group" "sg-itca" {
  name        = "sg-itca"
